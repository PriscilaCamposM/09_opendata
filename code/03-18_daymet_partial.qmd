...
#install.packages("sf") #manipulate geo-spatial formats
#install.packages("daymetr") #access data sets from other websites
#install.packages("remotes") #install packages that are not in CRAN (CRAN is stuff for R). when we install packages, the source is CRAN
#remotes::install_github("ropensci/USAboundaries") #packages that are only in github
#remotes::install_github("ropensci/USAboundariesData") #it has political boundary. 
...
```
#create a map of the US, and add the information
```{r}
#create boundary of the US
state <- us_states() %>%
...
#create a map
ggplot() +
  geom_sf(data=state) + #to plot sf project like the one we just created(states)
...
In R, we'll use a package called `daymetr` that facilitates making queries to Daymet API.  
